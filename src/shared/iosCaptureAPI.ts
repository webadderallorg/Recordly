...
export interface IOSCaptureAPI {
	...
	prepare(input: { deviceToken: string; generation: number; options: IOSRecordingOptions }): Promise<IOSCaptureSnapshot>;
	...
	onPreview(callback: (frame: { generation: number; sequence: number; jpeg: Uint8Array }) => void): () => void;
