...
export function registerRecordingHandlers(
	...
	ipcMain.handle(
		"set-recording-state",
		(_, recording: boolean, options?: { showKeystrokes?: boolean }) => {
			if (recording) {
				stopCursorCapture();
				...
				setIsKeystrokeCaptureActive(Boolean(options?.showKeystrokes));
				setActiveCursorSamples([]);
				setPendingCursorSamples([]);
				setActiveKeystrokeSamples([]);
				setPendingKeystrokeSamples([]);
	...
	ipcMain.handle("get-keystroke-telemetry", async (_, videoPath?: string) => {
		...
		const keystrokePath = getKeystrokePathForVideo(targetVideoPath);
		try {
			const content = await fs.readFile(keystrokePath, "utf-8");
