...
Initial classifier rule: a candidate must advertise muxed media and video and match an experimentally verified iOS-screen model signature. Start with the observed `iOS Device` model signature; any extra signature requires recorded hardware evidence and a fixture. A device's user-assigned name, its portrait dimensions, or `.external` alone is never sufficient. Ordinary Continuity Camera lens feeds remain excluded. Never use a sole arbitrary camera as fallback.
